...
namespace InterTrips___Turistička_Agencija.Controllers;
    ...
    public async Task<IActionResult> GetMojeRezervacije()
    {
        try
        {
            ...
            {
                return BadRequest(new { greska = "Korisnik nije prepoznat u sistemu (Ulogujte se ponovo)." });
            }
     ...
     .FirstOrDefaultAsync(k => k.Email == emailKorisnika);
            ...
            if (korisnik == null)
            {
                return BadRequest(new { greska = "Korisnik nije pronađen." });
            }
            ...
            var rezultat = siroveRezervacije.Select(r => new
            {
                ...
                putnici = (r.Putnici ?? new List<Putnik>()).Select(p => new
                {
                    ...
                    datumRodjenja = p.DatumRodjenja.HasValue
                        ? p.DatumRodjenja.Value.ToString("dd.MM.yyyy")
                        : "—",
        ...
        {
            return BadRequest(new { greska = ex.Message, unutrasnjaGreska = ex.InnerException?.Message });
        }
    }
}